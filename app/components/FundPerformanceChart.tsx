...
export const FundPerformanceChart = ({
  ...
  const chartData = useMemo(() => {
    ...
    console.log('Chart input data:', { tvl, percentageChange, data, creationDate, activeStreamsCount });
    ...
    let startDate = creationDate ? new Date(creationDate) : new Date(today.getTime() - 3 * 24 * 60 * 60 * 1000);
    ...
    const startValue = activeStreamsCount && activeStreamsCount > 0 
    ...
    for (let i = 0; i <= days; i++) {
      ...
      const value = Math.max(
        startValue * 0.9,
        Math.min(baseValue + noise, safeTvl * 1.1)
      );
