...
import {
  ...
  CFAv1ForwarderABI,
  USDCxABI,
  SUPERFLUID_QUERY_URL,
} from '@/app/config/superfluid';
...
export function useSuperfluid(fundAddress?: `0x${string}`) {
  ...
  const [aggregatedStreamData, setAggregatedStreamData] = useState<AggregatedStreamData | null>(
    null
  );
  ...
  const createStream = useCallback(
    async (receiverAddress: `0x${string}`, monthlyAmount: string): Promise<`0x${string}`> => {
      ...
      try {
        ...
        logger.log('Creating stream with flowRate:', {
          flowRate,
          to: receiverAddress,
          timestamp: Date.now(),
        });
